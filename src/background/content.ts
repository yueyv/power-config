...
chrome.runtime.onConnect.addListener((port) => {
  if (port.name === BACKGROUND_CONTENT_CONNECTION_NAME) {
    ...
    if (tabId) {
      ...
      console.log(`创建content script 连接，标签页 ID: ${tabId}，当前连接数: ${contentPorts.size}`);
 ...
 */
export function broadcastToAllContent(message: BackgroundConnectionMessage) {
 ...
 */
export function sendMessageToContent(tabId: number, message: BackgroundConnectionMessage) {
 ...
 * 向当前活动标签页发送消息
 */
export function sendMessageToActiveTab(message: BackgroundConnectionMessage) {
 ...
 * 获取所有连接的标签页 ID
 */
export function getConnectedTabIds(): number[] {
